...
module.exports = class Player extends Majiang.Player {
    ...
    qipai(qipai) {
        ...
        this._suanpai.qipai(
            qipai, (this._id + 3 - this._model.qijia + 3 - qipai.jushu) % 3);
        super.qipai(qipai);
    }
    zimo(zimo, gangzimo) {
    ...
    select_hule(data, hupai, info) {
        ...
        if (data) {
            ...
            let d = ['', '+', '=', '-'][(3 + data.l - this._menfeng) % 3];
    ...
    select_fulou(dapai, info) {
        ...
        let d = ['', '+', '=', '-'][(3 + dapai.l - this._menfeng) % 3];
        ...
        if (n_xiangting < 3) {
            ...
            const mianzi = this.get_peng_mianzi(this.shoupai, p) || [];
            if (!mianzi.length) return;
            ...
            if (info) {
                ...
                const tp0 = Majiang.Util.tingpai(this.shoupai) || [];
                let n_tingpai = tp0
            ...
            for (let m of mianzi) {
                ...
                if (info) {
                    if (!paishu) paishu = this._suanpai.paishu_all();
                    let ev = this.eval_shoupai(shoupai, paishu);
                    const tp1 = Majiang.Util.tingpai(shoupai) || [];
                    let n_tingpai = tp1
                        .map(p => this._suanpai._paishu[p[0]][p[1]])
                        .reduce((x, y) => x + y, 0);
                    info.push({
                ...
                if (x > min_xiangting) continue;

                let tingpai = Majiang.Util.tingpai(shoupai);
                if (!tingpai.length) continue;
                ...
                else if (n_tingpai > max_tingpai) {
                    max_tingpai = n_tingpai;
                    fulou = m;
                }
            }

            if (min_xiangting >= n_xiangting) return;
            return fulou;
        }
    }
    ...
    select_gang(info) {
        ...
        for (let m of this.get_gang_mianzi(this.shoupai)) {
            ...
            if (Majiang.Util.xiangting(shoupai) == n_xiangting) gang.push(m);
        }
        if (!gang.length) return;

        if (n_xiangting == -1) {
            if (this.select_lizhi()) return;
            return gang[0];
        }
        if (n_xiangting == 0) {
            for (let m of gang) {
                if (this.xiangting(this.shoupai.clone().gang(m)) == 0) return m;
            }
            return;
        }

        if (this.shoupai.lizhi) {
            const paishu = this._suanpai.paishu_all();
            const ev = this.eval_shoupai(this.shoupai, paishu);
            for (let m of gang) {
                let shoupai = this.shoupai.clone().gang(m);
        ...
        for (let m of gang) {
            let shoupai = this.shoupai.clone().gang(m);
            const ev = this.eval_shoupai(shoupai, paishu);

            if (info) {
                info.push({
                    m: m, n_xiangting: n_xiangting, ev: ev,
                    shoupai: shoupai.toString()
                });
            }
